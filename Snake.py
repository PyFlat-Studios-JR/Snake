...
class Snake_Manager:
    ...
    def change_dir(self, x, y):
        if self.gameloop and (abs(x) != abs(self.dir[0]) or abs(y) != abs(self.dir[1])):
            self.dir = [x, y]
    ...
    def add_tile(self):
        if self.length_of_snake > len(self.snake):
            self.obj = self.w.create_rectangle(
                self.x, self.y, self.x + 20, self.y + 20, fill="green"
            )
...
class Apple:
    ...
    def ini():
        ...
        Apple.obj = w.create_rectangle(
            Apple.x, Apple.y, Apple.x + 20, Apple.y + 20, fill="red"
        )
...
master.bind("<Up>", lambda ev: s.change_dir(0, -1), s.update())
master.bind("<Left>", lambda ev: s.change_dir(-1, 0), s.update())
master.bind("<Down>", lambda ev: s.change_dir(0, 1), s.update())
master.bind("<Right>", lambda ev: s.change_dir(1, 0), s.update())
